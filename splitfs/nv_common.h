...
// functions to use when invoking system calls (since the normal ones may be
// aliased)
// #define ALLOPS_FINITEPARAMS READ WRITE CLOSE SEEK TRUNC DUP DUP2 FORK MMAP
// READV WRITEV PIPE MUNMAP MSYNC #define ALLOPS OPEN IOCTL ALLOPS_FINITEPARAMS
...
#define ALLOPS_FINITEPARAMS_WPAREN                                                                                                                                               \
  (POSIX_FALLOCATE)(POSIX_FALLOCATE64)(                                                                                                                                          \
      FALLOCATE)(READ)(FREAD)(CLEARERR)(FEOF)(FERROR)(WRITE)(FWRITE)(FSEEK)(FTELL)(FTELLO)(CLOSE)(FCLOSE)(SEEK)(FTRUNC)(DUP)(DUP2)(FORK)(VFORK)(READV)(WRITEV)(PIPE)(SOCKETPAIR)(CHID_EXIT) \
      OPS_FINITEPARAMS_64(PREAD)(PWRITE)(                                                                                                                                        \
          FSYNC)(FDSYNC)(SOCKET)(ACCEPT)(UNLINK)(UNLINKAT)(SYNC_FILE_RANGE)(STAT)(STAT64)(FSTAT)(FSTAT64)(LSTAT)(LSTAT64)

#define ALLOPS_WPAREN                                                                                    \
  (OPEN)(OPENAT)(                                                                                        \
      CREAT)(EXECVE)(EXECVP)(EXECV)(FOPEN)(FOPEN64)(FREAD_UNLOCKED)(IOCTL)(TRUNC)(MKNOD)(MKNODAT)(FCNTL) \
...
#define FILEOPS_WITH_FP                                                        \
  (FREAD)(CLEARERR)(FEOF)(FERROR)(FWRITE)(FSEEK)(FTELL)(FTELLO)

#else

#define ALLOPS_FINITEPARAMS_WPAREN                                                  \
  (READ)(WRITE)(                                                                    \
      CLOSE)(SEEK)(FTRUNC)(DUP)(DUP2)(FORK)(VFORK)(READV)(WRITEV)(PIPE)(SOCKETPAIR)(CHID_EXIT) \
      OPS_FINITEPARAMS_64(PREAD)(PWRITE)(                                           \
          FSYNC)(FDSYNC)(SOCKET)(ACCEPT)(UNLINK)(UNLINKAT)(STAT)(STAT64)(FSTAT)(FSTAT64)(LSTAT)(LSTAT64)
#define ALLOPS_WPAREN                                                          \
  (OPEN)(OPENAT)(CREAT)(EXECVE)(EXECVP)(EXECV)(IOCTL)(TRUNC)(MKNOD)(MKNODAT)   \
      ALLOPS_FINITEPARAMS_WPAREN
#define SHM_WPAREN (SHM_COPY)
#define METAOPS (MKDIR)(RENAME)(LINK)(SYMLINK)(RMDIR)(SYMLINKAT)(MKDIRAT)
...
#define FILEOPS_WITH_FD                                                        \
  (READ)(WRITE)(                                                               \
      SEEK)(READV)(WRITEV)(FTRUNC)(FTRUNC64)(SEEK64)(PREAD)(PREAD64)(PWRITE)(PWRITE64)(FSYNC)(FDSYNC)(POSIX_FALLOCATE)(POSIX_FALLOCATE64)(FALLOCATE)(SYNC_FILE_RANGE)(FSTAT)(FSTAT64)
...
// Every time a function is used, determine whether the module's functions have
// been resolved.
#include <errno.h>
#include <fcntl.h>
#include <string.h>
...
// These aren't resolved at runtime before main() because it's impossible to
// determine RELIABLY which constructor will be called first.
#define CHECK_RESOLVE_FILEOPS_OLDVERSION(NAME)                                 \
  do {                                                                         \
...
#define CHECK_RESOLVE_FILEOPS(NAME)                                            \
  do {                                                                         \
    if (UNLIKELY(NAME##fileops == NULL)) {                                     \
      MSG("Tried to use " #NAME                                                \
          " fileops, but they weren't initialized!  BLARG\n" FAIL);            \
      assert(0);                                                               \
    }                                                                          \
  } while (0)
...
#define INIT_FILEOPS_P(NAME, PREFIX, already_initialized)                      \
  struct Fileops_p *fo =                                                       \
      (struct Fileops_p *)calloc(1, sizeof(struct Fileops_p));                 \
  fo->name = NAME;                                                             \
...
#define DUMP_FLAGS(flags, x)                                                   \
  do {                                                                         \
    if (FLAGS_INCLUDE(flags, x)) {                                             \
      DEBUG_P("%s(0x%X) ", #x, x);                                             \
    }                                                                          \
  } while (0)
...
#define INTF_CHID_EXIT int status 
...
#define INTF_SYNC_FILE_RANGE                                                   \
  int file, off_t offset, off_t nbytes, unsigned int flags
...
// These functions are used to manage the standard directory of available
// functions. This directory lives in _hub_.
...
#define DECLARE_AND_ALIAS_FUNCTS_IWRAP(r, data, elem)                          \
  DECLARE_AND_ALIAS_FUNCS(elem, data)

// Same as above, but used to declare without aliasing for all the functions in
// a module. _hub_ is the only module which actually does external aliasing.
#define DECLARE_WITHOUT_ALIAS_FUNCS(FUNCT, prefix)                             \
  RETT_##FUNCT prefix##FUNCT(INTF_##FUNCT);
#define DECLARE_WITHOUT_ALIAS_FUNCTS_IWRAP(r, data, elem)                      \
  DECLARE_WITHOUT_ALIAS_FUNCS(elem, data)

// Used to fill in the blanks for functions which aren't implemented in a
// module. The module's .c file should have a list of functions which aren't
// implemented which gets passed in.
#define WRAP_NOT_IMPLEMENTED(op, prefix)                                       \
  RETT_##op prefix##op(INTF_##op) {                                            \
    DEBUG("CALL: " MK_STR(prefix##op) " not implemented!\n");                  \
    assert(0);                                                                 \
  }
#define WRAP_NOT_IMPLEMENTED_IWRAP(r, data, elem)                              \
  WRAP_NOT_IMPLEMENTED(elem, data)
